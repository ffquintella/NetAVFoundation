		...
		internal class ObjCType {
			...
			public string CategoryName {
				get {
					...
					var name = attrib.ToString() ?? Registrar.GetTypeFullName (Type);
...
#error Unknown platform
		...
		ObjCType RegisterCategory (TType type, CategoryAttribute attrib, ref List<Exception> exceptions)
		{
			...
			if (attrib.GetType() is null) {
				AddException (ref exceptions, ErrorHelper.CreateError (4151, Errors.MT4151, GetTypeFullName (type)));
				return null;
			}

			var declaringType = RegisterType (attrib.GetType(), ref exceptions);
			if (declaringType is null) {
				AddException (ref exceptions, ErrorHelper.CreateError (4150, Errors.MT4150, GetTypeFullName (type), GetTypeFullName (attrib.GetType())));
				return null;
			}
			...
			Trace ("    [CATEGORY] Registering {0} on {1}", type.ToString ().Replace ('+', '/'), attrib.GetType().FullName);
