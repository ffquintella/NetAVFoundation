...
#pragma warning restore CS8618
		...
		static IntPtr GetClassHandle (Type type, bool throw_if_failure, out bool is_custom_type)
		{
			...
			*/
			...
			@class = objc_getClass (type.Name);
			
			if(!is_custom_type && @class == IntPtr.Zero && throw_if_failure)
				throw ErrorHelper.CreateError (8026, $"Cant get classhandle for {type.FullName}");
		...
		unsafe static IntPtr FindClass (Type type, out bool is_custom_type)
		{
			...
			var ctypeAttr =
				(ClassTypeAttribute) Attribute.GetCustomAttribute(type, typeof (ClassTypeAttribute));
			
			if(ctypeAttr != null)
			{
				if(ctypeAttr.TopType != null)
				{
					type = ctypeAttr.TopType;
				}
				is_custom_type = (ctypeAttr.TypeFlags) == Runtime.MTTypeFlags.CustomType;
			...
			for (int i = 0; i < map->map_count; i++) {
				...
				{
					is_custom_type = (ctypeAttr.TypeFlags) == Runtime.MTTypeFlags.CustomType;
				}else is_custom_type = (class_map.flags & Runtime.MTTypeFlags.CustomType) == Runtime.MTTypeFlags.CustomType;
